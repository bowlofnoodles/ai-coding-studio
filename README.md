...
## 🤔 为什么不直接用现有的 AI Coding SaaS 平台？

市面上已有不少 AI 编码平台，但大多是 SaaS 服务，对于企业内部使用存在几个关键限制：

- 🔒 **代码安全** — 私有部署，代码和 Token 不出内网，不经过第三方服务器
- 🔌 **对接内部基础设施** — 可集成内部 GitLab、内部发布系统、内部沙箱平台，不受平台能力边界限制
- 🤖 **AI 引擎可替换** — 不绑定单一 AI 供应商，可切换模型或接入内部 AI 工具，支持第三方代理降低成本
- 🎨 **非技术人员友好** — 不需要理解 Git、PR、CI/CD 这些概念，一句话描述就能看到效果
- ⚙️ **行为可定制** — 通过 CLAUDE.md 精确控制 AI 的 Git 工作流、编码规范、提交策略
